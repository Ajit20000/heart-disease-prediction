...
def main():
    ...
    if(Gender=="Yes"):
        ...
        Gender=0
    Age = int(st.number_input('Age:',step=None))
    ...
    if st.button("submit"):
        output=classifier.predict([[int(Gender),int(Age),int(Smoke),float(Ciggerates),float(Bp),int(stroke),int(hyp),int(diab),float(totchol),float(sbp),float(dbp),float(bmi),float(HeartRate),float(glucose)]])
        if output[0]==1:
